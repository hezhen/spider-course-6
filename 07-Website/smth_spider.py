...
# Get all boards
for i in range(0,10):
    c = boards_crawler.get_pagination_content(i)
    boards += boards_crawler.get_board_list(c)

    # Get article list of board
    for board in boards:
        url = boards['board_url']
        content = topics_crawler.get_pagination_content(url, 1)
        articles = topics_crawler.get_article_list(content)
        total_pages_of_board = topics_crawler.get_total_pages(content)
